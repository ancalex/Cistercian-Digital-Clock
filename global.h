...
void WriteConfig(){
  ...
  // Application Settings here... from EEPROM 192 up to 511 (0 - 511)
  WriteStringToEEPROM(192, config.solidColor);
  EEPROM.write(202, config.effect);
  EEPROM.commit();
}
...
boolean ReadConfig(){
  ...
  if (EEPROM.read(0) == 'C' && EEPROM.read(1) == 'F'  && EEPROM.read(2) == 'G' )
  {
    ...
    config.solidColor =ReadStringFromEEPROM(192);
    config.effect = EEPROM.read(202);
    return true;
  }
  else
  {
